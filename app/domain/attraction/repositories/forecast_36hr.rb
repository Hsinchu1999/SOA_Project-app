...
module TravellingSuggestions
  module Repository
    # An Repository object for Entity::Forecast36Hr
    class Forecasts36Hr
      def self.find_id(id)
        rebuild_entity Database::Forecast36hrOrm.first(id:)
      end

      def self.find_region_id(region_id)
        rebuild_entity Database::Forecast36hrOrm.first(region_id:)
      end

      def self.rebuild_entity(db_record)
        return nil unless db_record

        first_12hr_entity = Repository::ForecastsPer12Hr.find_id(db_record.forecast_first_12hr_id)
        second_12hr_entity = Repository::ForecastsPer12Hr.find_id(db_record.forecast_second_12hr_id)
        third_12hr_entity = Repository::ForecastsPer12Hr.find_id(db_record.forecast_last_12hr_id)
      ...
      def self.rebuild_many_entities(db_records)
        db_records.map do |db_member|
          Repository::Forecasts36Hr.rebuild_entity(db_member)
        end
      end
      ...
      def self.db_find_or_create(region_entity)
        ...
        if db_forecast_36hr.forecast_first_12hr_id.nil?
          db_first_12hr = Database::Forecastper12hrOrm.create(
            region_entity.weather.forecast_36hr.first_12hr.to_attr_hash
          )
        ...
        if db_forecast_36hr.forecast_second_12hr_id.nil?
          db_second_12hr = Database::Forecastper12hrOrm.create(
            region_entity.weather.forecast_36hr.second_12hr.to_attr_hash
          )
          db_forecast_36hr.update(forecast_second_12hr_id: db_second_12hr.id)
        else
          db_second_12hr = db_forecast_36hr.forecast_second_12hr
          db_second_12hr.update(
            region_entity.weather.forecast_36hr.second_12hr.to_attr_hash
          )
        end
        if db_forecast_36hr.forecast_last_12hr_id.nil?
          db_last_12hr = Database::Forecastper12hrOrm.create(
            region_entity.weather.forecast_36hr.third_12hr.to_attr_hash
          )
